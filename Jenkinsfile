...
podTemplate(
    ...
    node(POD_LABEL) {
        ...
        stage('Upload Bar File') {
            container("oc-deploy") {
                withCredentials([usernamePassword(credentialsId: 'nexus_dev', usernameVariable: 'ARTIFACTORY_USER', passwordVariable: 'ARTIFACTORY_PASSWORD')]) {
                    sh label: '', script: '''#!/bin/bash
                        set -e
                        cd $PROJECT_DIR
                        ls -lha
